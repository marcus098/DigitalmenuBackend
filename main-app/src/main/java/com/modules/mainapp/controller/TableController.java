...
public class TableController {
    ...
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_WAITER')")
    @PostMapping("/update")
    public ResponseEntity<DataResponse<List<TableDto>>> updateTable(@RequestBody UpdateTables updateTable) {
        List<TableDto> table = tableService.updateTable(updateTable);
        return ResponseEntity.status(table == null ? 400 : 200).body(new DataResponse<List<TableDto>>(null, table));
    }
