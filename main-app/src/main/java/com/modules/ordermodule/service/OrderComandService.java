...
public class OrderComandService {
    ...
    public String addOrderFromClient(AddComandClient request) {
        try {
            Optional<com.modules.common.dto.TableDto> tableOpt = tableUtils.findById(request.getTableId());
            if (tableOpt.isEmpty()) return null;
            com.modules.common.dto.TableDto table = tableOpt.get();
            if (!table.isBusy() || table.getSessionId() == null || table.getSessionId().isEmpty()) return null;

            long idAgency = table.getIdAgency();
            String tableSessionId = table.getSessionId();
            String comandId = UUID.randomUUID().toString() + "_" + System.currentTimeMillis();

            List<Order> orders = orderList(idAgency, 0L, request.getOrders(), comandId);
            ComandFromWaiterJpa comand = new ComandFromWaiterJpa(idAgency, request.getTableId(), 0L, ComandStatus.AWAIT, orders, tableSessionId);
            comand.setComandWaiterType(com.modules.common.model.enums.ComandWaiterType.TABLE);

            Comand saved = mongoComandRepository.save(comand);
            sendOrderKafkaEvent(saved.getId(), ComandStatus.AWAIT.toString(), idAgency);
            return saved.getId();
        } catch (Exception e) {
    ...
    public String addPublicTakeaway(long idAgency, String customerName, String customerPhone, String pickupTime, List<AddComandOrder> orders) {
        try {
            ...
            comand.setComandWaiterType(com.modules.common.model.enums.ComandWaiterType.TAKE_AWAY);
            Comand saved = mongoComandRepository.save(comand);
            sendOrderKafkaEvent(saved.getId(), ComandStatus.PENDING.toString(), idAgency);
            return saved.getId();
        } catch (Exception e) {
    ...
    public CompletableFuture<Integer> changeStatusToComand(String idComand, ComandStatus comandStatus) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                Long idAgencyFromComand = comandFromWaiterJpa.getIdAgency();
                ...
                allOf.join();
                sendOrderKafkaEvent(idComand, comandStatus.toString(), idAgencyFromComand);
